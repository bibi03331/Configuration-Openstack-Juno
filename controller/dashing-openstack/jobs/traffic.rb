...
cmd_get_status = "ssh " + environments['user_name'] + '@' + environments['host'] + " ceph osd pool stats -f json 2>&1"
...
SCHEDULER.every '2s' do
  ...
  if poolstats_available
    ...
    cmd_get_status = "ssh " + environments['user_name'] + '@' + environments['host'] + " timeout 2 ceph osd pool stats -f json"
    result = `#{cmd_get_status}`
    ...
    cmd_get_status = "ssh " + environments['user_name'] + '@' + environments['host'] + " timeout 2 ceph status -f json"
    result = `#{cmd_get_status}`
