...
RAW_TOKEN=`curl -s -d "$REQUEST" -H "Content-type: application/json" "http://10.0.0.30:5000/v2.0/tokens"`
TOKEN=`echo $RAW_TOKEN | python -c "import sys; import json; tok = json.loads(sys.stdin.read()); print tok['access']['token']['id'];"`

curl -s -H "X-Auth-Token: $TOKEN" http://$CONTROLLER_HOST:8774/v2/$1/os-services # | python -mjson.tool
